...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
    ...
    app.post('/api/recommendations', async (c) => {
        const { sessionId, recommendation } = await c.req.json();
        if (!recommendation || !sessionId) {
            return c.json({ success: false, error: 'Missing recommendation or sessionId' }, { status: 400 });
        }
        const controller = getAppController(c.env);
        await controller.saveRecommendation(sessionId, recommendation);
        return c.json({ success: true });
    });
    app.get('/api/recommendations/:sessionId', async (c) => {
        const sessionId = c.req.param('sessionId');
        const controller = getAppController(c.env);
        const rec = await controller.getRecommendation(sessionId);
        return c.json({ success: !!rec, data: rec });
    });
    app.get('/api/recommendations', async (c) => {
        const controller = getAppController(c.env);
        const recs = await controller.listRecommendations();
        return c.json({ success: true, data: recs });
    });
    app.delete('/api/recommendations/:sessionId', async (c) => {
        const sessionId = c.req.param('sessionId');
        const controller = getAppController(c.env);
        const deleted = await controller.deleteRecommendation(sessionId);
